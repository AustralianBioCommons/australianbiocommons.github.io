...
QCIF_UQ <- read_tsv("../../external_GitHub_inputs/systems.tsv") %>%
  arrange(`System name`) %>%
  ...
  mutate(
    QCIF = case_when(
      grepl("^https", QCIF_url) ~ paste0("<a href='", QCIF_url, "'>", QCIF, "</a>")),
    UQ = case_when(
      grepl("^https", UQ_url) ~ paste0("<a href='", UQ_url, "'>", UQ, "</a>"))
  ) %>%
  mutate(`Partner organisation` = paste(QCIF, UQ, sep = " | ")) %>%
  select(`Partner organisation`, State, `System name`, Cloud, HPC, System_link)
  
systems <- read_tsv("../../external_GitHub_inputs/systems.tsv") %>%
  filter(!grepl("\\|", `Partner organisation`)) %>%
  mutate(
    `Partner organisation` = case_when(
      grepl("^https", URL) ~ paste0("<a href='", URL, "'>", `Partner organisation`, "</a>"))
    ) %>%
  select(-URL) %>%
  bind_rows(QCIF_UQ) %>% 
  arrange(`System name`) %>%
  mutate(
    `System name` = case_when(
      grepl("^https", System_link) ~ paste0("<a href='", System_link, "'>", `System name`, "</a>"))
  )
