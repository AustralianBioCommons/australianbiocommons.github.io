...
container1 = htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(class = 'dt-center', style="font-size:11pt", colspan = 4, 'Availability on Australian compute infrastructures')
    ),
    tr(
      lapply(c("Name", "Homepage", "Registry link", "Tool identifier (e.g. module name)",
               ...
               "Resources / documentation", "Galaxy Australia", "NCI (Gadi)", "NCI (if89)", "Pawsey (Setonix)"), th)
    )
  )
))
...
options = list(
            ...
            columnDefs = list(list(visible=FALSE, targets = c(3,7:8)),
                              list(className = 'dt-center', targets = c(0:12)))
            ),
